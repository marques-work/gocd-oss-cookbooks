...
choco install -y nodejs.install --version="${env:NODEJS_VERSION}"
...
Invoke-WebRequest https://s3.amazonaws.com/mirrors-archive/local/perforce/r${env:P4_VERSION}/bin.ntx64/p4.exe -Outfile "C:\\Program Files (x86)\\Perforce\\bin\\p4.exe"
Invoke-WebRequest https://s3.amazonaws.com/mirrors-archive/local/perforce/r${env:P4D_VERSION}/bin.ntx64/p4d.exe -Outfile "C:\\Program Files (x86)\\Perforce\\bin\\p4d.exe"
Invoke-WebRequest https://github.com/ketan/gocd-golang-bootstrapper/releases/download/${env:GOLANG_BOOTSTRAPPER_VERSION}/go-bootstrapper-${env:GOLANG_BOOTSTRAPPER_VERSION}.windows.amd64.exe -Outfile C:\\go-agent.exe
